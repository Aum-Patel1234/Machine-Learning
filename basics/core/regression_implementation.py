...
class Regression2Variables:
    ...
    def get_variance_or_deviation(self, series: pd.Series, deviation=False, get_population_variance=True , populationAndSample = False) -> float:
        try:
    ...
    def find_line_of_regression(self, x:pd.Series, y:pd.Series):
        ...
        r = self.get_coefficient_of_correlation(x,y)        # we need this for calculation as it is a 2 variable regression
        ...
        if(sqrt(bxy*byx) > 1):                                                                           # r = sqrt(byx*bxy)  and  r cannot be greater than 1
            return "cannot calculate the prediction as the r > 0, try flipping the columns"
...
def main():
    ...
    population_variance,sample_variance = my_regression_class.get_variance_or_deviation(df.iloc[:, 0],populationAndSample=True)

    print(f'My variance = {population_variance} and the numpy variance is - {np.var(df.iloc[:, 0].astype(float))} \n\n pandas variance = {df.iloc[: ,0].var()} and the sample variance = {sample_variance}\n\nHence numpy calculates by dividing by n (population_variance) whereas pandas calculate dividing by n-1 (sample_variance)!!\n\n')

    print(f'r = {my_regression_class.get_coefficient_of_correlation(df.iloc[:, 0], df.iloc[:, 1])} and the in built r = {df.corr()}\n\n')
